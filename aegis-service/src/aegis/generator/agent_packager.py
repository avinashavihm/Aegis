...
RUN pip install --no-cache-dir -r requirements.txt || echo "Some dependencies may have failed to install"

# Copy and install Aegis framework if it exists
COPY aegis ./aegis
RUN if [ -d "./aegis" ] && [ -f "./aegis/pyproject.toml" ] || [ -f "./aegis/setup.py" ]; then \\
        pip install -e ./aegis || echo "Aegis framework installation failed, continuing"; \\
    else \\
        echo "Aegis framework not found or incomplete, skipping installation"; \\
    fi
...
services:
  {project.name.replace('_', '-')}:
    ...
    container_name: aegis-{project.name.replace('_', '-')}
    image: aegis-{project.name}:latest
    environment:
      - PYTHONUNBUFFERED=1
...
# OS - macOS resource forks and metadata (must exclude to avoid xattr errors)
._*
.DS_Store
.DS_Store?
.Spotlight-V100

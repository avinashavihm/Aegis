...
async def list_api_keys(user_id: UUID = Depends(get_current_user_id)):
    """List all API keys for the current user"""
    user_id_str = str(user_id)
    with get_db_connection(user_id_str) as conn:
...
async def create_api_key(data: APIKeyCreate, user_id: UUID = Depends(get_current_user_id)):
...
async def get_api_key(key_id: UUID, user_id: UUID = Depends(get_current_user_id)):
...
async def update_api_key(key_id: UUID, data: APIKeyUpdate, user_id: UUID = Depends(get_current_user_id)):
    """Update an API key"""
    user_id_str = str(user_id)
    with get_db_connection(user_id_str) as conn:
        with conn.cursor() as cur:
            ...
            if data.is_default is not None:
                ...
                if data.is_default:
                    ...
                    if row:
                        ...
                        """, (user_id_str, row['provider'], str(key_id)))
            ...
            params.extend([str(key_id), user_id_str])
...
async def delete_api_key(key_id: UUID, user_id: UUID = Depends(get_current_user_id)):
    """Delete an API key"""
    user_id_str = str(user_id)
    with get_db_connection(user_id_str) as conn:
        with conn.cursor() as cur:
...
async def test_api_key(key_id: UUID, user_id: UUID = Depends(get_current_user_id)):
...
async def list_providers(user_id: UUID = Depends(get_current_user_id)):
...
async def list_agent_topics(agent_id: UUID, user_id: UUID = Depends(get_current_user_id)):
    ...
    with get_db_connection(user_id_str) as conn:
        with conn.cursor() as cur:
            ...
            """, (str(agent_id), data.name, data.description, data.trigger_type,
                  ...
                  data.is_enabled, data.priority, user_id_str))
            conn.commit()
            row = cur.fetchone()
            return dict(row)
...
async def list_agent_actions(agent_id: UUID, user_id: UUID = Depends(get_current_user_id)):
...
async def list_agent_knowledge(agent_id: UUID, user_id: UUID = Depends(get_current_user_id)):
    ...
    with get_db_connection(user_id_str) as conn:
        with conn.cursor() as cur:
            ...
            """, (str(agent_id), data.name, data.source_type, data.source_url, 
                  data.content, data.sync_frequency, data.is_enabled, user_id_str))
            conn.commit()
            row = cur.fetchone()
            return dict(row)
